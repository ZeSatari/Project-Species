## =====================================================================
## 50_run_msom_sim_v3.R -- replicated multi-species simulation
## =====================================================================
## DIAGNOSTIC RUN. This does NOT supersede v2, which remains the primary
## result; the manuscript reports both side by side in Table 6 and
## describes this run in Appendix S1.2 as a diagnostic. Confirm on load
## that the banner reports "v3 | homogeneous slopes".
##
## CHANGE FROM v2: species-specific slopes are switched off
## (sd_beta1 = 0), so every species shares the community slope exactly.
##
## WHY. Under v2 the slopes were drawn with sd = 0.5 about a mean of 0.7,
## and coverage for the community slope came out at 0.68 against a
## nominal 0.95. Diagnostics established that the shortfall is not a
## failure of the fit:
##
##   - removing the shared field, the species-specific slopes, or the
##     sum-to-zero constraint left the discrepancy essentially unchanged
##     (0.610, 0.583, 0.595 against a generating value of 0.7);
##   - quadrupling the sample made it WORSE, not better (0.663 at 200
##     sites, 0.610 at 800), which rules out finite-sample bias;
##   - generating with sd_beta1 = 0 recovered 0.677 (0.648, 0.706),
##     covering the generating value.
##
## The reason is that under a non-linear link the slope identified from
## aggregated data is a marginal quantity, not the arithmetic mean of the
## species-specific slopes. A delta-method approximation puts the
## marginal slope near 0.667 for sd = 0.5, which accounts for roughly a
## third of the gap; the remainder we were unable to attribute.
##
## Rather than report coverage against a target the model does not
## estimate, we remove the ambiguity at source. THE COST IS REAL and must
## be stated in the manuscript: with homogeneous slopes, the simulation no
## longer tests the model's ability to recover species-specific
## environmental responses. That capability is exercised in the empirical
## application but is not validated here.
## =====================================================================
...
OUTDIR <- "results/msom"
record_session(OUTDIR, tag = "homogeneous")
...
## sd_beta1 = 0: all species share the community slope exactly, so the
## generating value and the estimand coincide. This is the single change
...
message("=== 50_run_msom_sim_v3 | homogeneous slopes | N_REP = ", N_REP,
...
## ---------------------------------------------------------------------
CHECKPOINT_DIR <- file.path(OUTDIR, "checkpoints_v3")
CHUNK <- 10L
...
saveRDS(list(comm = comm_df, species = sp_df, shrink = shrink_df),
        file.path(OUTDIR, "msom_replicates_v3.rds"))
...
write.csv(shrink_summary, file.path(OUTDIR, "shrinkage_v3.csv"), row.names = FALSE)
