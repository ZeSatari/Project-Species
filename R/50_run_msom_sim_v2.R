## =====================================================================
## 50_run_msom_sim_v2.R -- multi-species simulation, PRIMARY RUN
## =====================================================================
## Heterogeneous species-specific slopes (sd_beta1 = 0.5). This is the
## PRIMARY multi-species result: it produces Table 6 row 1, Table S4 and
## Table S5 of the manuscript.
##
## It is NOT superseded by the other two runs in this directory. They
## answer different questions and all three are reported:
##   this file  heterogeneous slopes   -> primary result
##   ..._v3.R   homogeneous slopes     -> diagnostic (Table 6 row 2, S1.2)
##   ..._v4.R   heterogeneous prevalence -> attempted run (Section 4.5)
##
## Confirm on load that the banner reports "v2 | linear covariate".
##
## Changes from v1:
##   - LINEAR environmental covariate (was x_s^2). With x ~ N(0,1), x^2
##     is chi-squared_1; its right tail drives psi toward one, where the
##     data carry little information about the slope. Diagnostics with no
##     spatial structure at n = 5000 gave beta1 = 0.697 (linear), 0.456
##     (x^2 raw) and 0.614 (x^2 standardised) against a truth of 0.700.
##     The attenuation had nothing to do with the hierarchical structure
##     this study evaluates.
##   - N_REP 200 -> 100, site_frac 0.20 -> 0.02, N_WORKERS -> 4
##   - the cross-validated MSOM/SSOM comparison is withdrawn; accuracy
##     against the known truth is measured instead
##   - constrained species effects, with the fixed term retained
##   - memory released within each replicate
## =====================================================================
...
OUTDIR <- "results/msom"
record_session(OUTDIR, tag = "main")
...
message("=== 50_run_msom_sim_v2 | LINEAR covariate | N_REP = ", N_REP,
...
saveRDS(list(comm = comm_df, species = sp_df, shrink = shrink_df),
        file.path(OUTDIR, "msom_replicates.rds"))
...
write.csv(shrink_summary, file.path(OUTDIR, "shrinkage.csv"), row.names = FALSE)
